...
export default function LoginClient({ schools }: { schools: { login_id: string; school_name: string }[] }) {
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  tabIndex={-1}
                >
                  {isAdminMode ? "학교 로그인으로 돌아가기" : "관리자이신가요?"}
                </button>
              </div>
              ...
              {isAdminMode ? (
                ...
                <select
                  ...
                  required
                  disabled={isLoading}
                  className="w-full px-4 py-3 rounded-xl bg-slate-900 border border-white/[0.12] text-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-amber-400/60 focus:border-amber-400/40 transition-all disabled:opacity-50 cursor-pointer"
                  style={{
                ...
                >
                  <option value="" disabled className="text-slate-400">-- 학교명을 선택해 주세요 --</option>
                  {schools.map(school => (
                    <option key={school.login_id} value={school.login_id} className="text-white">
                      {school.school_name}
                    </option>
                  ))}
                </select>
